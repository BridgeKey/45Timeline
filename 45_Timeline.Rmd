...
data3$Age <- as.numeric(max(data3$Date)-data3$Date)

data4 <- list(NULL)
for (i in 1:nrow(data3)){
data4[[i]] <- data.frame(EventDate = rep(data3$Date[i],data3$Age[i]),
                Date = rep(data3$Date[i],data3$Age[i]) + 0:(data3$Age[i]-1),
                Text = rep(data3$Text[i],data3$Age[i]),
                Category = rep(data3$Category[i], data3$Age[i]))
}
data5 <- do.call("rbind",data4)

data5$Month <- monthStart(data5$Date)
data6 <- data5
data7 <- data6[!duplicated(data6[,c("Text", "Month")]),]
data7$Event <- gsub('(.{1,90})(\\s|$)', '\\1\n',data7$Text)
...
p <- ggplot(data7) + 
  geom_bar(aes(x=Month,
               ...
               label= EventDate),
           width = 25) + 
  ggtitle("A Cumulative Timeline of President Trump's Actions against the United States") +
  theme_solarized() +
  xlab(NULL)

ggplotly(p, dynamicTicks = T, tooltip = c("label", "Event")) %>%
  rangeslider()
